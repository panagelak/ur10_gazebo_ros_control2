...
def generate_launch_description():
    ...
    moveit_cpp_yaml_file_name = os.path.join(
        get_package_share_directory('ur10_moveit_demo_nodes'),
        'config',
        'moveit_cpp.yaml')
    ...
    robot_description_semantic_config = load_file('ur10_moveit_config',
                                                  os.path.join('config', 'ur10.srdf'))
    robot_description_semantic = {'robot_description_semantic': robot_description_semantic_config}

    kinematics_yaml = load_yaml('ur10_moveit_config',
                                os.path.join('config', 'kinematics.yaml'))

    controllers_yaml = load_yaml('ur10_moveit_demo_nodes',
    ...
    rviz_config_file = os.path.join(
        get_package_share_directory('ur10_gazebo'), 'config', 'moveitcpp.rviz')
    ...
    return LaunchDescription([
      IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(get_package_share_directory('ur10_gazebo'), 'launch'), '/ur10_gazebo.launch.py'
        ]),
      ),
      run_moveit_cpp_node,
      rviz_node,
